/*******************************************************************
 * RCSId: $Id: rotorfuncs.ino,v 1.1 2021/07/29 08:25:38 ralblas Exp $
 *
 * Project: rotordrive
 * Author: R. Alblas
 *
...
void run_to_endswitch(ROTOR *AX_rot, ROTOR *EY_rot,int speed)
{
  ...
  if (AX_rot)
  {
    ax_maxspeed=CMDP(AX_rot,maxSpeed());
    CMDP(AX_rot,setMaxSpeed(ax_maxspeed*speed/100.));

    CMDP(AX_rot,setCurrentPosition(0));
    AX_rot->req_degr=RUN_ENDSW_MAX;
    AX_rot->at_end1=false;
    AX_rot->at_end2=false;
  }
  if (EY_rot)
  {
    ey_maxspeed=CMDP(EY_rot,maxSpeed());
    CMDP(EY_rot,setMaxSpeed(ey_maxspeed*speed/100.));

    CMDP(EY_rot,setCurrentPosition(0));
    EY_rot->req_degr=RUN_ENDSW_MAX;
